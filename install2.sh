...
s1=$(cat $pd|grep ext4|awk '{print $4}'|tr -dc '0-9')
#FAT32 size in sectors
s2=$(cat $pd|grep fat32|awk '{print $4}'|tr -dc '0-9')
...
echo The EXT4 partition size is $s1 sectors '(' $s1gb Gb ')'
echo The FAT32 partition size is $s2 sectors '(' $s2gb Gb ')'
